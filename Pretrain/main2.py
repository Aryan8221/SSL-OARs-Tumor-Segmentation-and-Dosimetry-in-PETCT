    ...
    def train(args, global_step, train_loader, val_best, scaler):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with autocast(enabled=args.amp):
                # Forward pass for x1_augment on cuda:0
                x1_augment = x1_augment.cuda(0)
                rot1_p, contrastive1_p, rec_x1 = model(x1_augment)

                # Move x1_augment to CPU to free GPU memory if necessary
                x1_augment_cpu = x1_augment.cpu()

                # Clear memory for x1_augment
                del x1_augment
                torch.cuda.empty_cache()

                # Forward pass for x2_augment on cuda:0
                x2_augment = x2_augment.cuda(0)  # Ensure x2_augment is also on cuda:0
                rot2_p, contrastive2_p, rec_x2 = model(x2_augment)

                # Move x1_augment back to cuda:0 when needed
                x1_augment = x1_augment_cpu.cuda(0)

                # Concatenate the outputs
                rot_p = torch.cat([rot1_p, rot2_p], dim=0)
                rots = torch.cat([rot1.to('cuda:0'), rot2.to('cuda:0')], dim=0)
            ...
            if args.distributed:
                if dist.get_rank() == 0:
                    logger.info(f"Step:{global_step}/{args.num_steps}, Loss:{loss.item():.4f}, Time:{time() - t1:.4f}")
            else:
                logger.info(f"Step:{global_step}/{args.num_steps}, Loss:{loss.item():.4f}, Time:{time() - t1:.4f}")
